...
function weekDaysAttendance()
{
    $data = [];

    for ($i = 0; $i <= 4; $i++) {
        $diaQuery = conectar()->query("SELECT * FROM asistencias WHERE WEEKDAY(AsistenciaFecha) = '$i' AND WEEK(AsistenciaFecha) = WEEK(NOW())");
        $dia = $diaQuery->rowCount();
        $data[] = $dia;
    }
